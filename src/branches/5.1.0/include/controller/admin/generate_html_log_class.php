...
final class JinrouAdminGenerateHTMLLogController extends JinrouAdminController {
  ...
  protected static function RunCommand() {
    //-- Validate --//
    if (RQ::Fetch()->min_room_no < 1) {
      $str = sprintf(GenerateHTMLLogMessage::INVALIDE_ROOM_START, RQ::Fetch()->min_room_no);
      return self::OutputResult($str);
    }

    if (RQ::Fetch()->max_room_no < 1 || RQ::Fetch()->min_room_no > RQ::Fetch()->max_room_no) {
      $str = sprintf(
	GenerateHTMLLogMessage::INVALIDE_ROOM_END,
	RQ::Fetch()->min_room_no, RQ::Fetch()->max_room_no
      );
      return self::OutputResult($str);
    }
  ...
  private static function GenerateRoom() {
    $format = sprintf('../%s/%s', GenerateHTMLLogConfig::DIR, RQ::Fetch()->prefix) . '%d%s.html';
    ...
    self::OutputResult(sprintf($format, RQ::Fetch()->min_room_no, RQ::Fetch()->max_room_no));
  }
