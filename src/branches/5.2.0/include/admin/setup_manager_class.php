<?php
//-- データベース初期セットアップマネージャ --//
final class JinrouSetupManager {
  ...
  private static function OutputHeader() {
    HTML::OutputHeader(SetupMessage::TITLE, close: true);
    DB::Enable(true, true);
  }

  //セットアップ実行
  private static function Run() {
    self::SetupDatabase(DatabaseConfig::NAME);
    foreach (DatabaseConfig::$name_list as $id => $name) {
      DB::Disconnect();
      self::SetupDatabase($name, $id);
    }
  }

  //DB構築
  private static function SetupDatabase(string $name, int $id = null) {
    Text::p($name, SetupMessage::TARGET_DB);
    if (null !== $id) {
      $id++;
    }

    if (false === DB::ConnectInHeader($id)) {
  ...
  //テーブル構築
  private static function SetupTable(string $name) {
    if (JinrouVersion::Newer(ServerConfig::REVISION, ScriptInfo::REVISION)) {
  ...
  //テーブル存在確認
  private static function Exists(string $table) {
    return in_array($table, self::$table_list);
  }

  //再構成処理
  private static function Reset(string $table) {
    switch ($table) {
    case 'count_limit':
      if (JinrouVersion::Older(863)) {
	self::DropTable($table);
      }
      break;

    case 'document_cache':
      if (JinrouVersion::Older(792) || JinrouVersion::Older(863)) {
	self::DropTable($table);
      }
      break;
    }
  ...
  //-- SQL 発行 --//
  //データベース作成
  private static function CreateDatabase(string $name) {
    SetupDB::Connect();
  ...
  //インデックス作成
  private static function CreateIndex(string $table, string $index, string $value) {
    $result = SetupDB::CreateIndex($table, $index, $value);
    self::OutputResult(SetupMessage::CREATE_INDEX, $table, $result);
  }

  //インデックス再生成
  private static function RegenerateIndex(string $table, string $index, string $value) {
  ...
  //テーブル文字コード変更
  private static function ChangeEncode(string $table, string $encode) {
    $title  = SetupMessage::CHANGE_ENCODE . $table;
    $result = SetupDB::ChangeEncode($table, $encode);
    self::OutputResult($title, $table, $result);
  }

  //型変更
  private static function ChangeColumn(string $table, array $column_list) {
  ...
  //テーブル削除
  private static function DropTable(string $table) {
  ...
  //カラム削除
  private static function DropColumn(string $table, string $column) {
  ...
  //初期データ登録
  private static function Insert(string $table) {
    switch ($table) {
    case 'user_entry':
      //管理者登録
      $list = [
  ...
  //更新処理
  private static function Update(string $table) {
    if (JinrouVersion::Older('#115')) {
      self::ChangeEncode($table, 'utf8mb4');
    }
  ...
  //結果出力
  private static function OutputResult(string $title, string $name, $result) {
