--------------------------------------------------------------------------------
  人狼式 ～汝は人狼なりや？式神エディション Version 5.2.0 α1 README 2025/12/10
--------------------------------------------------------------------------------
...
◆設定ファイル関連
・Ver. 5.0.0 (パッケージバージョン) / Rev. #28 (GitHub Pull Request 番号)
管理用の設定ファイル置き場が config/admin に新設されました。
管理機能を ON にするとトップページに警告メッセージが出力されるようになりました。
...
・Ver. 5.2.0 α1 (パッケージバージョン) / Rev. #127 (GitHub Pull Request 番号)
上記のバージョン未満の場合は、DB の文字コードが変更されます。

・Ver. 3.0.0 α1 (パッケージバージョン) / Rev. 1051 (SourceForgeリビジョン番号)
上記のバージョン未満の場合は、発言数制限用のテーブルが作成されます。

・Ver. 2.3.0 α1 (パッケージバージョン) / Rev. 883 (SourceForgeリビジョン番号)
上記のバージョン未満の場合は、カラムの型の全体的な再設計が実施されます。
データベースのデータ量が多い場合は setup.php の実行時間が長くなるので注意してください。
...
◆文字コード関連
・Ver. 5.2.0 α1 (パッケージバージョン) / Rev. #127 (GitHub Pull Request 番号)
上記のバージョン以降、データベースの文字コードを UTF-8 から UTF-8 mb4 に変更しています。
admin/setup.php を実行することで自動で変換されます。
...
├─config     : 設定ファイル
├─css        : CSS ファイル
├─dev        : 開発用ファイル (テスト用なのでアップロード不要です)
├─img        : システム画像ファイル
├─include    : システムライブラリファイル
├─info       : 説明書などの情報ファイル
├─javascript : JavaScript ファイル
├─log        : HTML 化ログ置き場 (変換していない場合は不要です)
├─module     : 外部モジュールファイル
├─sound      : 音声ファイル
├─top        : TOP ページ表示用 HTML ファイル
└─user_icon  : アイコン画像ファイル

------------
  動作環境
------------
本スクリプトは、PHP8＋MySQL8 環境下での動作を想定しています。
MySQL 以外の RDBS 環境を用いる場合、MySQL 用クエリ関数を書き直す必要があります。

注：
PHP4 環境のサポートは Ver. 1.4.x までとなっています。
Ver. 1.5 以降は PHP5 環境が必要です。
Ver. 4.0 以降は PHP5.6 環境が必要です。
Ver. 5.0 以降は PHP8.2 環境が必要です。

------------
  設置方法
------------
 1. MySQL サーバ内に専用データベースとアカウントを作成
    文字コード、照合順序は utf8mb4_unicode_ci (注) を使用すること
 2. config/server/database_config.php を編集し、データベースを設定
 3. config/server/server_config.php を編集し、サーバを設定
 4. config/admin/admin_config.php を編集し、全体設定と初期設定管理ツールを有効に設定
 5. パッケージをサーバにアップロード (コード無変換、またはバイナリモード)
 6. サーバ上の user_icon/、rss/ ディレクトリに書き込み属性を付与
 7. admin/ ディレクトリにアクセス制限を設定
 8. 初期セットアップ作業 (自動) のために admin/setup.php にアクセス
 9. index.php にアクセスし、動作を確認
10. 必要に応じてデザイン編集 (その他の情報一覧 → 管理者向け情報)
11. 4. で設定した内容を無効化して再度アップロード
注：
Ver. 5.2.0 α1 より、文字コードを utf8_unicode_ci から utf8mb4_unicode_ci に変更しています
...
config/admin/admin_config.php を編集し、全体設定を有効にした上で個々の設定を有効にする必要があります。
...
削除用リンクから呼び出されるものです。
ただし、admin/ ディレクトリにアクセス制限をかけていない場合、有効化すると
直接アドレス入力することで、任意の村の削除が誰にでも可能となります。
そのため、通常稼動時には OFF に設定しておくようにしてください。
デフォルトでは OFF に設定されています。

有効化すると、トップページに警告メッセージが表示されます。
意図していない場合は設定を確認してください。
警告メッセージの表示有無も個別に設定できます。
...
1. admin/   : 主に管理用スクリプトの設定ファイル置き場
2. server/  : 主にサーバ側の設定ファイル置き場
3. game/    : 主にゲーム内の設定ファイル置き場
4. system/  : 主にシステム関連の設定ファイル置き場
5. message/ : 主にゲーム内のメッセージ設定ファイル置き場

主な設定ファイルは以下になります
1-1. admin/admin_config.php     : 管理ツールの設定ファイル
2-1. server/database_config.php : データベース関連の設定ファイル
2-2. server/server_config.php   : 本スクリプトをサーバ上で動作させるための設定ファイル
2-3. server/room_config.php     : 村作成時の制限設定ファイル
3-1. game/game_config.php       : ゲーム内の設定ファイル
3-2. game/cast_config.php       : 配役設定ファイル
3-3. game/message.php           : ゲーム中で表示されるシステムメッセージの設定ファイル
4-1. system/version.php         : トップページで表示されるバージョン情報の設定ファイル

このうち、必ず設定が必要になるのは admin_config.php, database_config, server_config.php のみです。
...
--------------------------
  デバッグモードについて
--------------------------
ON にすることで開発テスト用のツール群が機能するようになり、一部の例外チェックがスキップされます。
...
user_icon テーブル中の該当レコード、user_icon/ ディレクトリ内の該当アイコン画像を削除してください。
...
・パスワードハッシュ化のための salt 文字列は十分に強固な文字列を設定してください。
...
Q. トップページに / でアクセスしても表示されない
A. index.html が事前に設置されてあることがありますので削除してください。

Q. 動いたけど文字化けしてる
A. MySQL サーバの設定、データベースの設定、ブラウザのエンコード、
   どれかが UTF-8 (utf8_unicode_ci) を使用していない可能性が高いです。
   一度チェックしてみてください。
   サーバによっては UTF-8 が使えない事があります。
   その場合はファイル自体の文字コードをサーバにあわせる必要があります。
   また、Ver. 5.2.0 α1 より、文字コードを utf8_unicode_ci から utf8mb4_unicode_ci に変更しています。

Q. ゲーム一覧に「MySQL サーバ接続失敗」が表示される
A. データベースの初期設定が正しく行われていません。
   ブラウザでサーバの admin/setup.php にアクセスしてください。

Q. 「このスクリプトは使用できない設定になっています。」と表示される
A. config/admin/admin_config.php の設定を確認してください。
...
Q. 発言欄の文字列が消えない
A. javascript/game_up.js に格納されている JavaScript 関数が機能していません。
   該当ファイルのアクセス権限や、ブラウザの設定を確認してください。

-------------------------
  公式配布 URL について
-------------------------
本スクリプトの公式配布 URL は以下です。
https://github.com/umethyl/jinrousiki
...
人狼式 ～汝は人狼なりや？式神エディション (GitHub プロジェクトページ)
https://github.com/umethyl/jinrousiki
...
Version 5.0.0:
・ベース php バージョンを 8.2 に変更
・ベース MySQL バージョンを 8.0 に変更
・管理機能の有効設定を個別化

Version 4.0.0:
・ベース php バージョンを 5.6 に変更
・ベースの html バージョンを html4 から html5 に変更
・音声再生フォーマットを swf から mp3 に変更
